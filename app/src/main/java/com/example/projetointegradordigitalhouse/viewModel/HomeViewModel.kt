...
import com.example.projetointegradordigitalhouse.model.characters.Data
...
internal class HomeViewModel: ViewModel() {
    ...
    var charList: MutableLiveData<List<Result>> = MutableLiveData()
        ...
        viewModelScope.launch {
            when (val response = homeBusiness.getCharacters()) {
                is ResponseApi.Success -> {
                    var data = response.data as Characters
                    charList.postValue(data.data.results)
